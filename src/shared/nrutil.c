...
#define M 7
#define NSTACK 50
void indexxui(unsigned int n, unsigned int *arr, unsigned int *indx) {
  unsigned int i, j, k, l;
  unsigned int indxt, itemp, ir;
  unsigned int *istack, jstack;
  unsigned int a;
...
void *new_vvector(unsigned long long nl, unsigned long long nh, enum alloc_type type) {
  ...
  case NRUTIL_UPTR2:
    ...
    break;
  case NRUTIL_XPTR:
    v = (struct snpAuxiliaryInfo **) gvector(nl, nh, sizeof(struct snpAuxiliaryInfo*)) -nl+NR_END;
    break;
  case NRUTIL_QPTR:
    v = (struct quantileObj **) gvector(nl, nh, sizeof(struct quantileObj*)) -nl+NR_END;
    break;
  case NRUTIL_QPTR2:
    v = (struct quantileObj ***) gvector(nl, nh, sizeof(struct quantileObj**)) -nl+NR_END;
    break;
  case NRUTIL_SPTR:
    v = (struct lookupInfo **) gvector(nl, nh, sizeof(struct lookupInfo*)) -nl+NR_END;
    break;
  case NRUTIL_SPTR2:
    v = (struct lookupInfo ***) gvector(nl, nh, sizeof(struct lookupInfo**)) -nl+NR_END;
    break;
...
#endif
  case NRUTIL_LEAFPTR:
    v = (struct leafLinkedObj **) gvector(nl, nh, sizeof(struct leafLinkedObj*)) -nl+NR_END;
    break;
  case NRUTIL_LEAFPTR2:
    v = (struct leafLinkedObj ***) gvector(nl, nh, sizeof(struct leafLinkedObj**)) -nl+NR_END;
    break;
  case NRUTIL_SRTLNKPTR:
    v = (struct sortedLinkedObj **) gvector(nl, nh, sizeof(struct sortedLinkedObj*)) -nl+NR_END;
    break;
  default:
    v = NULL;
...
void free_new_vvector(void *v, unsigned long long nl, unsigned long long nh, enum alloc_type type) {
  ...
  case NRUTIL_NPTR3:
    ...
    break;
  case NRUTIL_FPTR:
    free_gvector((struct factor**) v +nl-NR_END, nl, nh, sizeof(struct factor*));
    break;
  case NRUTIL_FPTR2:
    free_gvector((struct factor***) v +nl-NR_END, nl, nh, sizeof(struct factor**));
    break;
  ...
  case NRUTIL_UPTR2:
    ...
    break;
  case NRUTIL_XPTR:
    free_gvector((struct snpAuxiliaryInfo **) v +nl-NR_END, nl, nh, sizeof(struct snpAuxiliaryInfo*));
    break;
  case NRUTIL_QPTR:
    free_gvector((struct quantileObj **) v +nl-NR_END, nl, nh, sizeof(struct quantileObj*));
    break;
  case NRUTIL_QPTR2:
    free_gvector((struct quantileObj ***) v +nl-NR_END, nl, nh, sizeof(struct quantileObj**));
    break;
  case NRUTIL_SPTR:
    free_gvector((struct lookupInfo **) v +nl-NR_END, nl, nh, sizeof(struct lookupInfo*));
    break;
  case NRUTIL_SPTR2:
    free_gvector((struct lookupInfo ***) v +nl-NR_END, nl, nh, sizeof(struct lookupInfo**));
    break;
...
#endif
  case NRUTIL_LEAFPTR:
    free_gvector((struct leafLinkedObj**) v +nl-NR_END, nl, nh, sizeof(struct leafLinkedObj*));
    break;
  case NRUTIL_LEAFPTR2:
    free_gvector((struct leafLinkedObj***) v +nl-NR_END, nl, nh, sizeof(struct leafLinkedObj**));
    break;
  case NRUTIL_SRTLNKPTR:
    free_gvector((struct sortedLinkedObj**) v +nl-NR_END, nl, nh, sizeof(struct sortedLinkedObj*));
    break;
  default:
